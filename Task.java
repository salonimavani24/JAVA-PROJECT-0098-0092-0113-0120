...
//added
    ...
    /**
     * Helper to set due date from string (format yyyy-MM-dd).
     * Returns true if parsed and set, false if parse failed.
     */
    public boolean setDueDateFromString(String dateStr) {
        if (dateStr == null || dateStr.trim().isEmpty()) {
            this.dueDate = null;
            return true;
        }
        try {
            LocalDate d = LocalDate.parse(dateStr.trim(), DATE_FORMAT);
            this.dueDate = d;
            return true;
    ...
    /**
     * Returns formatted due date or empty string if not set.
     */
    public String getDueDateString() {
        return (dueDate == null) ? "" : dueDate.format(DATE_FORMAT);
    }

    /**
     * Returns true if this task is overdue (dueDate before today).
     * If no due date, returns false.
     */
