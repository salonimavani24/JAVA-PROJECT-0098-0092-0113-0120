...
public class TodoList {
    ...
    try {
        ...
        // ----- UPDATED DUE DATE HANDLING -----
        System.out.print(">>> Due Date [example: 2019-12-31] or press Enter to skip: ");
        String dueDateInput = scan.nextLine().trim();
        ...
        if (!dueDateInput.isEmpty()) {
            try {
                ...
                System.out.println("Invalid date format! Task will be added without a due date.");
                dueDate = null;
            }
        }

        // create task with title, project, and optional due date
        this.taskList.add(new Task(title, project, dueDate));
...
//
    ...
    public void listAllTasks(String sortBy) {
        ...
        if (sortBy.equals("2")) {
            ...
            taskList.stream()
                    .sorted(Comparator.comparing(task -> task.getDueDate().orElse(LocalDate.MAX)))
...
public int overdueCount() {
    int count = 0;
    // adjust according to how tasks are stored (List<Task> tasks, or Map, etc.)
    // Example if you have List<Task> tasks:
    for (Task t : this.taskList) {
        if (t != null && t.isOverdue()) count++;
    }
    return count;
}
//
