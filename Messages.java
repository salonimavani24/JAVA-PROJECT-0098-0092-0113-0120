...
public static void listTasks(List<Task> tasks) {
    int index = 1;
    for (Task task : tasks) {
        String due = task.getDueDateString(); // "" if none
        String overdueTag = task.isOverdue() ? " OVERDUE!" : "";
        System.out.printf("%d) [%s] %s", index, task.isComplete() ? "x" : " ", task.getTitle());
        if (!due.isEmpty()) {
            System.out.printf(" (Due: %s)%s", due, overdueTag);
        }
        System.out.println();
        index++;
    }
...
//added// Add this method to the Messages class
public static void showDashboard(int completed, int pending, int overdue) {
    ...
    // header
    System.out.println();
    System.out.println("=== Progress Summary Dashboard ===");

    // summary lines with emojis
    System.out.println(" Completed: " + completed + " tasks");
    System.out.println(" Pending:   " + pending + " tasks");
    System.out.println(" Overdue:   " + overdue + " tasks");
...
//added
public static void showOverdueSummary(int overdueCount) {
    if (overdueCount > 0) {
        System.out.println();
        System.out.println(" You have " + overdueCount + " overdue task" + (overdueCount > 1 ? "s" : "") + "!");
        System.out.println();
    } else {
        System.out.println();
        System.out.println(" No overdue tasks! Keep it up!");
        System.out.println();
    }
}
//
